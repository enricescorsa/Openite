...
_(darrera actualització 19/03/2020)_
...
Represento les 3 variables: nombre de casos i nombre de casos en els darrers 15 dies en l'eix _x_, l'eix _y_ respectivament i el nombre de morts el represento amb la mida del cercle.
...
g<-ggplot(taula_coronavirus) +
  geom_point(aes(x=taula_coronavirus$"Casos confirmats els darrers 15 dies", y=taula_coronavirus$"Casos confirmats", size = taula_coronavirus$"Morts"*100, fill=Regio, text = Pais), alpha = 0.5, show.legend = FALSE) + scale_size(range = c(0.5, 25)) + labs(title="Distribució geogràfica del COVID-19", x = "Nous casos confirmats en els darrers 15 dies", y = "Casos confirmats") + theme_minimal()
#He assignat un color per Regió (Continent)

#genero el gràfic interactiu amb plotly
ggplotly(g,tooltip = c("Pais")) %>% 
  layout(showlegend=FALSE, hovermode="x")
```
